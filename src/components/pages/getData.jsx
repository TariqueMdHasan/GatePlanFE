              ...
              {TodoList.filter((todo) => todo.status.toUpperCase() === status).map((todo) => (
                <div
                  ...
                  className={`min-h-30 w-full border-2 rounded-2xl mt-2 flex justify-center pl-4 p-2 flex-col cursor-grab
                    ${status === "TODO" ? "bg-teal-200 border-teal-400" :
                ...
                >
                  {editingTodo?._id === todo._id ? (
                    <>
                      ...
                      <select
                        value={editingTodo.scheduledIn}
                        onChange={(e) =>
                          setEditingTodo({ ...editingTodo, scheduledIn: e.target.value })
                        }
                        className="border p-1 rounded mb-2"
                      >
...
//   // ✅ New function
//   const moveExpiredTasksToBacklog = (todos) => {
//     const today = new Date();
...
//       if (taskDate < today && todo.status.toLowerCase() !== "done") {
...
//         const processedTodos = moveExpiredTasksToBacklog(newTodos);

//         const cachedData = localStorage.getItem("todosData");
//         if (!cachedData || JSON.stringify(processedTodos) !== cachedData) {
//           localStorage.setItem("todosData", JSON.stringify(processedTodos));
//           setShowUpdated(true);
//           setTimeout(() => setShowUpdated(false), 2000);
//         }

//         setTodoList(applyFilter(processedTodos));
//       } else {
//         toast.error("Data Not Found");
//       }
//     } catch (error) {
...
//       const processed = moveExpiredTasksToBacklog(parsed);
//       setTodoList(applyFilter(processed));
//     }
...
//   const handleStatusChange = async (id, newStatus) => {
//     try {
//       await axios.put(`${TASK_API}update/${id}`, {
//         status: newStatus.toLowerCase(),
//       });
...
//   const handleEditSave = async (todo) => {
//     try {
//       await axios.put(`${TASK_API}update/${todo._id}`, {
//         status: todo.status.toLowerCase(),
//         date: todo.date,
//         scheduledIn: todo.scheduledIn,
//       });
...
//   const handleDelete = async () => {
//     try {
//       await axios.delete(`${TASK_API}delete/${selectedTodo._id}`);
//       toast.success("Todo deleted");
//       setShowDeleteModal(false);
//       setSelectedTodo(null);
//       fetchData();
...
//             <div className="w-full flex-1 overflow-y-auto flex flex-col gap-4 p-4 relative">
//               {TodoList.filter(
//                 (todo) => todo.status.toUpperCase() === status
//               ).map((todo) => (
//                 <div
//                   key={todo._id}
//                   draggable
...
//                           className={`text-[10px] px-4 py-[1px] rounded-2xl border-2 text-white uppercase 
//                           ${
//                             todo.scheduledIn === "morning"
//                               ? "bg-orange-400 border-orange-600"
//                               : todo.scheduledIn === "afternoon"
